...
import type { Financeiro, TipoFaturamento } from "@/types";
...
>(function FinanceiroFields({ financeiroInicial }, ref) {
  ...
  const [marcos, setMarcos] = useState<{ descricao: string; valor: string }[]>(
    tipoInicial === "marco_faturamento" && (financeiroInicial?.parcelas.length ?? 0) > 0
      ? financeiroInicial!.parcelas.map((p) => ({ descricao: p.descricao ?? "", valor: String(p.valor) }))
      : [{ descricao: "", valor: "" }]
  );
  ...
  function adicionarMarco() {
    setMarcos((prev) => [...prev, { descricao: "", valor: "" }]);
  }
